                ...
                <div class="userformdata">
                    ...
                    <div class="form-group-flex">
                        ...
                        <div class="form-group custom-form-group">
                            ...
                            <input type="text" required class="form-control" v-model="uploadUser.phone" placeholder="Enter Phone" />
...
export default {
	data() {
		return {
			currentID: window.location.href.split("/")[6],
			hasAvatar: false,
			fileImgPreview: {
				src: ''
			},
	...
	methods: {
		gettingCurrentUser() {
			axios.get('/betterhr/userlist/currentuser/' + this.currentID)
			.then((resp) => {
				this.uploadUser = resp.data.data;
				console.log(resp);
				this.fileImgPreview.src = resp.data.data.avatar;
				this.hasAvatar = true;
			})
			.catch((error) => {
		...
		createRecordOne() {
			...
			if(
				...
				this.errors = {
					...
					email: (this.uploadUser.email == "") ? 'Enter your account email' : '',
				...
				axios.post('/betterhr/userlist/edit/' + this.currentID, data, headers)
				.then((resp) => {
					document.querySelector('#closeNow').click();
					if(resp.data.status) {
